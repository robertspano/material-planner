...
export async function PATCH(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (contentType.includes("multipart/form-data")) {
      ...
      if (description !== null && formData.has("description")) updateData.description = description;
      ...
      if (isActive !== null && formData.has("isActive")) updateData.isActive = isActive === "true";

      const tileWidth = formData.get("tileWidth") as string | null;
      if (tileWidth) updateData.tileWidth = parseFloat(tileWidth);

      const tileHeight = formData.get("tileHeight") as string | null;
      if (tileHeight) updateData.tileHeight = parseFloat(tileHeight);

      const tileThickness = formData.get("tileThickness") as string | null;
      if (tileThickness) updateData.tileThickness = parseFloat(tileThickness);
